...
object TimeUsage {
  ...
  def read(resource: String): (List[String], DataFrame) = {
    ...
    //dataFrame.createOrReplaceTempView("tt")
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val fields = columnNames.tail.map(StructField(_, DoubleType, nullable = false))
  ...
  def row(line: List[String]): Row = {
    val firstValue = line.head
    val otherValues = line.tail.map(_.toDouble)
    Row fromSeq firstValue +: otherValues
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryPattern = "^((?:t01|t03|t11|t1801|t1803).*)".r
    val workingPattern = "^((?:t05|t1805).*)".r
    val otherPattern = "^((?:t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).*)".r
    ((List[Column](), List[Column](), List[Column]()) /: columnNames) {
      case ((primary, working, other), primaryPattern(columnName)) => (primary :+ col(columnName), working, other)
      case ((primary, working, other), workingPattern(columnName)) => (primary, working :+ col(columnName), other)
      case ((primary, working, other), otherPattern(columnName)) => (primary, working, other :+ col(columnName))

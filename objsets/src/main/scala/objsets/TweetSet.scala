...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    try {
      val a = (this remove elem).mostRetweeted
      if (a.retweets > elem.retweets) a else elem
    } catch {case _: NoSuchElementException => elem}
  }
...
object GoogleVsApple {
  val google = List("android", "Android", "galaxy", "android", "nexus", "Nexus")
  val apple = List("ios", "iOS", "iphone", "iPhone", "ipad", "iPad")

  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(containsAcc(_, google))

  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(containsAcc(_, apple))

  @tailrec
  def containsAcc(tweet: Tweet, acc: List[String]): Boolean =
    if (acc.isEmpty) false
    else if (tweet.text.contains(acc.head)) true
    else containsAcc(tweet, acc.tail)
